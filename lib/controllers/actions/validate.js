...
module.exports = function ( req, res ) {
    ...
    try {
        ...
        sails.models.auth.findOne( _token.iss ).populate( 'validateToken' ).exec( function ( err, auth ) {
            ...
            var localauth = _.findWhere( sails.config.waterlock.authMethod, { name: 'waterlock-local-auth' } );
            
            if ( localauth && localauth.validateAccount && localauth.validateAccount.redirectOnValidateUrl ) {
                res.redirect( localauth.validateAccount.redirectOnValidateUrl );
            } else {
                res.json( 200 );
            }
